...
def get_tokenizer(model_name_or_path, fast_tokenizer=True):
    if "llama" in model_name_or_path:
        from my_transformers.models.llama import LlamaTokenizer
...
# This function is a modified version of code available in the from_pretrained API of HuggingFace my_transformers
# The code is copied and modified from: https://github.com/huggingface/my_transformers/blob/5ee9693a1c77c617ebc43ef20194b6d3b674318e/src/my_transformers/modeling_utils.py#L498
...
def save_zero_three_model(model_ema, global_rank, save_dir, zero_stage=0, max_shard_size=1024 * 1024 * 1024):
    zero_stage_3 = (zero_stage == 3)
    ...
    if not zero_stage_3:
        ...
        if global_rank == 0:
            ...
            for key, value in state_dict.items():
                ...
                if shard_size + tensor_size > max_shard_size and shard:
                    shard_file = os.path.join(save_dir, f"pytorch_model-{shard_index:05d}-of-00000.bin")
                    torch.save(shard, shard_file)
                    index["weight_map"].update({k: os.path.basename(shard_file) for k in shard.keys()})
                    shard = {}
                    shard_size = 0
                    shard_index += 1
            ...
            index["metadata"] = {"total_shards": total_shards}
            index_file = os.path.join(save_dir, "pytorch_model.bin.index.json")
            with open(index_file, "w") as f:
                json.dump(index, f, indent=2)

            print(f"Model saved with {total_shards} shards to {save_dir}")
