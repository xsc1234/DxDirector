...
@app.route("/event_stance", methods=["POST"])
def event_stance():
    ...
    # 执行搜索查询
    response = es.search(index="sc_event_posting", body=query)
    user_stances = []
    ...
    # 返回处理后的数据
    return jsonify({'event_stance':event_stance, "stance_stability":stability, 'statusCode': 200, 'data': "#115420"})
    

if __name__ == '__main__':
    app.run(host="0.0.0.0", port=18067, debug=True)
