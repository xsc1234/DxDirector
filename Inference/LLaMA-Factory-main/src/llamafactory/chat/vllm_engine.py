...
class VllmEngine(BaseEngine):
    def __init__(
        ...
        self.max_pos_length = model_args.vllm_maxlen
    ...
    async def _generate(
        ...
        **input_kwargs,
        ...
        if 'QwQ' in self.model_name:
            ...
            model_inputs = self.tokenizer(messages, return_tensors="pt")
            prompt_ids = model_inputs['input_ids']
            prompt_ids = prompt_ids[0].tolist()
            #print('after: ',prompt_ids)
        else:
        ...
        #stop = [r"\[Answer \d+\]"]
        if not 'QwQ' in self.model_name:
            stop = '[Answer'
        #print('stop is {}'.format(stop))
        print('max_new_tokens is {}'.format(max_new_tokens))
